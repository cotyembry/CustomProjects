...
#Commonly known as screen scraping
...
foreach $current_url_in_the_array (@url_array) {
	...
	#make sure the file name has a .mp3 at the end of it
	if ($file_name =~ m/\.mp3/gi) {
		#print"has correct file extension";
	}
	else {
	...
	if($the_file_already_exists == 0) {
		print"about to download: $url_to_download\n";
